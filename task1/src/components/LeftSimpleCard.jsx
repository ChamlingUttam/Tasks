...
const LeftSimpleCard = ({ textContent, heading, subHeading, bg, imageUrl }) => {
  return (
    <div className={`relative flex rounded-4xl p-6 overflow-hidden ${bg}`}>
      {/* Image pinned to bottom-left */}
      {imageUrl && (
        <div className="shrink-0 relative w-44">
          <img
            className="animate-float w-44 h-64 absolute -bottom-6 -left-6 object-contain"
            src={imageUrl}
            alt="card visual"
          />
        </div>
      ...
      {/* Right content */}
      <div className="flex-1 pt-5 pl-4">
        <h1 className="font-bold text-4xl text-white text-end pr-8">{heading}</h1>
        <h3 className="font-semibold text-white text-2xl pt-4 pb-7 text-end pr-8">{subHeading}</h3>
        <p className="text-white pb-10 text-end pr-8 text-xl">{textContent}</p>
      </div>
